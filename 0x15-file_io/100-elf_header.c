#include <stdlib.h>
#include <stdio.h>
#include <unistd.h>
#include <fcntl.h>
#include <sys/stat.h>
#include <sys/types.h>
#include <elf.h>

void check_el(unsigned char *e_ident);
void print_magi(unsigned char *e_ident);
void print_clas(unsigned char *e_ident);
void print_dat(unsigned char *e_ident);
void print_versio(unsigned char *e_ident);
void print_ab(unsigned char *e_ident);
void print_osab(unsigned char *e_ident);
void print_typ(unsigned int e_type, unsigned char *e_ident);
void print_entr(unsigned long int e_entry, unsigned char *e_ident);
void close_el(int elf);

/**
 * check_el - verifies if elf
 * @e_ident: pointer to numbers
 */
void check_el(unsigned char *e_ident)
{
	int inde;

	for (inde = 0; inde < 4; inde++)
	{
		if (e_ident[inde] != 127 &&
				e_ident[inde] != 'E' &&
				e_ident[inde] != 'L' &&
				e_ident[inde] != 'F')
		{
			dprintf(STDERR_FILENO, "Error: Not an ELF file\n");
			exit(98);
		}
	}
}
/**
 * print_magi - prints magic numbers
 * @e_ident: pointer to numbers
 */
void print_magi(unsigned char *e_ident)
{
	int inde;

	printf("  Magic:   ");
	for (inde = 0; inde < EI_NIDENT; inde++)
	{
		printf("%02x", e_ident[inde]);
		if (inde == EI_NIDENT - 1)
			printf("\n");
		else
			printf(" ");
	}
}
/**
 * print_clas - prints class
 * @e_ident: pointer to num
 */
void print_clas(unsigned char *e_ident)
{
	printf("  Class:                             ");
	switch (e_ident[EI_CLASS])
	{
		case ELFCLASSNONE:
			printf("none\n");
			break;
		case ELFCLASS32:
			printf("ELF32\n");
			break;
		case ELFCLASS64:
			printf("ELF64\n");
			break;
		default:
			printf("<unknown: %x>\n", e_ident[EI_CLASS]);
	}
}
/**
 * print_dat - prints data
 * @e_ident: pointer
 */
void print_dat(unsigned char *e_ident)
{
	printf("  Data:                              ");
	switch (e_ident[EI_DATA])
	{
		case ELFDATANONE:
			printf("none\n");
			break;
		case ELFDATA2LSB:
			printf("2's complement, little endian\n");
			break;
		case ELFDATA2MSB:
			printf("2's complement, big endian\n");
			break;
		default:
			printf("<unknown: %x>\n", e_ident[EI_CLASS]);
	}
}
/**
 * print_versio - prints version
 * @e_ident: pointer
 */
void print_versio(unsigned char *e_ident)
{
	printf("  Version:                           %d", e_ident[EI_VERSION]);
	switch (e_ident[EI_VERSION])
	{
		case EV_CURRENT:
			printf(" (current)\n");
			break;
		default:
			printf("\n");
			break;
	}
}
/**
 * print_osab - prints osabi
 * @e_ident: pointer
 */
void print_osab(unsigned char *e_ident)
{
	printf("  OS/ABI:                            ");
	switch (e_ident[EI_OSABI])
	{
	case ELFOSABI_NONE:
		printf("UNIX - System V\n");
		break;
	case ELFOSABI_HPUX:
		printf("UNIX - HP-UX\n");
		break;
	case ELFOSABI_NETBSD:
		printf("UNIX - NetBSD\n");
		break;
	case ELFOSABI_LINUX:
		printf("UNIX - Linux\n");
		break;
	case ELFOSABI_STANDALONE:
		printf("Standalone App\n");
		break;
	case ELFOSABI_ARM:
		printf("ARM\n");
		break;
	case ELFOSABI_TRU64:
		printf("UNIX - TRU54\n");
		break;
	case ELFOSABI_FREEBSD:
		printf("UNIX - FreeBSD\n");
		break;
	case ELFOSABI_IRIX:
		printf("UNIX - IRIX\n");
		break;
	case ELFOSABI_SOLARIS:
		printf("UNIX - Solaris\n");
		break;
	default:
		printf("<unknown: %x>\n", e_ident[EI_OSABI]);
	}
}
/**
 * print_ab - prints abi
 * @e_ident: pointer
 */
void print_ab(unsigned char *e_ident)
{
	printf("  ABI Version:                       %d\n", e_iden[EI_ABIVERSION]);
}
/**
 * print_typ - prints type
 * @e_type: Type
 * @e_ident: pointer
 */
void print_typ(unsigned int e_type, unsigned char *e_ident)
{
	if (e_ident[EI_DATA] == ELFDATA2MSB)
		e_type >>= 8;
	printf("  Type:                              ");
	switch (e_type)
	{
		case ET_NONE:
			printf("NONE (None)\n");
			break;
		case ET_REL:
			printf("REL (Relocatable file)\n");
			break;
		case ET_EXEC:
			printf("EXEC (Executable file)\n");
			break;
		case ET_DYN:
			printf("DYN (Shared object file)\n");
			break;
		case ET_CORE:
			printf("CORE (Core file)\n");
			break;
		default:
			printf("<unknown: %x>\n", e_type);
	}
}
/**
 * print_entr - prints entry point
 * @e_entry: adress of entry
 * @e_ident: pointer
 */
void print_entr(unsigned long int e_entry, unsigned char *e_ident)
{
	printf("  Entry point address:               ");
	if (e_ident[EI_DATA] == ELFDATA2MSB)
	{
		e_entry = ((e_entry << 8) & 0xFF00FF00) |
			((e_entry >> 8) & 0xFF00FF);
		e_entry = (e_entry << 16) | (e_entry >> 16);
	}
	if (e_ident[EI_CLASS] == ELFCLASS32)
		printf("%#x\n", (unsigned int)e_entry);
	else
		printf("%#lx\n", e_entry);
}
/**
 * close_el - close file
 * @el: file
 */
void close_el(int el)
{
	if (close(el) == -1)
	{
		dprintf(STDERR_FILENO, "Error: Can't close fd %d\n", el);
		exit(98);
	}
}
/**
 * main - prints info of ELF file
 * @argc: counter
 * @argv: vector
 * Return: 0
 */
int main(int __attribute__((__unused__))argc, char *argv[])
{
	Elf64_Ehdr *head;
	int a, s;

	a = open(argv[1], O_RDONLY);
	if (a == -1)
	{
		dprintf(STDERR_FILENO, "Error: Can't read file %s\n", argv[1]);
		exit(98);
	}
	head = malloc(sizeof(Elf64_Ehdr));
	if (head == NULL)
	{
		close_el(a);
		dprintf(STDERR_FILENO, "Error: Can't read file %s\n", argv[1]);
		exit(98);
	}
	s = read(a, head, sizeof(Elf64_Ehdr));
	if (s == -1)
	{
		free(head);
		close_el(a);
		dprintf(STDERR_FILENO, "Error: `%s`: No such file\n", argv[1]);
		exit(98);
	}
	check_el(head->e_ident);
	printf("ELF Header:\n");
	print_magi(head->e_ident);
	print_clas(head->e_ident);
	print_dat(head->e_ident);
	print_versio(head->e_ident);
	print_osab(head->e_ident);
	print_ab(head->e_ident);
	print_typ(head->e_type, head->e_ident);
	print_entr(head->e_entry, head->e_ident);

	free(head);
	close_el(a);
	return (0);
}
